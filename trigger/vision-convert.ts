...
PHOTO CAPTION EXCLUSION RULE (STRUCTURAL/NUMBERED VERSION)
Some source documents number their real inventory rows within each room section, with the number appearing on its own line, directly followed by that row's item/description text on the next line(s) - for example "146." on one line, then "Chrome gripper strip" on the next line, together forming ONE real numbered row (item 146). Do not treat a standalone number line as separate from the text that immediately follows it - they belong together as a single row.
Text that appears BEFORE the first number line of a room, with no number anywhere near it, is a photo caption or gallery label - NOT a genuine inventory item. Exclude only this unnumbered leading text entirely. Only begin extracting real inventory items starting from the first numbered row onward, and every number plus its following text forms one real row that must be kept.
Example: if a room's content reads "Wall Angle Rubbed / Oven / Hob / 146. / Chrome gripper strip / 147. / Ceiling: Painted white stipple artex / Yellow stain RHS..." - only the three unnumbered leading entries (Wall Angle Rubbed, Oven, Hob) are photo captions to exclude. "146. Chrome gripper strip" and "147. Ceiling..." are both genuine numbered rows and must be kept in full.
This rule takes priority over any instinct to include every visible word - but it only excludes text that appears BEFORE any number line in a room. Never exclude text that has a number line immediately before it, no matter how short that text is.
If a document does NOT use numbered rows at all, fall back to visual context: a short isolated cluster of words positioned near a photograph rather than inside the structured inventory table, immediately after "Further Views" and before the real populated table data begins, is still very likely a photo caption and should be excluded on the same basis. This is different from a genuine parent/heading row (like "Back Wall" or "Facing Wall") which legitimately has blank description/condition because it is a real structural row within the actual inventory table, not a photo label.
