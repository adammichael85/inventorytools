...
PHOTO CAPTION EXCLUSION RULE (STRUCTURAL/NUMBERED VERSION): Some source documents number their real inventory rows within each room section, with the number appearing on its own line, directly followed by that row's item/description text on the next line(s) - for example "146." on one line, then "Chrome gripper strip" on the next line, together forming ONE real numbered row (item 146). Do not treat a standalone number line as separate from the text that immediately follows it - they belong together as a single row.
...
If a document does NOT use numbered rows at all, fall back to context: a short isolated cluster of words immediately after "Further views" with no description or condition, appearing before the real populated table data begins, is still very likely a photo caption and should be excluded on the same basis.
