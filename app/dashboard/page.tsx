          ...
          {page === 'reports' && (
            <div>
              <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 14, overflow: 'hidden' }}>
                    ...
                    {conversions.filter(c => !searchQuery || (c.address||'').toLowerCase().includes(searchQuery.toLowerCase())).map(conv => (
                      <div key={conv.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderBottom: `1px solid ${BORDER}` }}>
                        ...
                        <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                          ...
                          <button onClick={() => conv.accuracy_report ? setViewingReport(conv) : conv.extracted_text ? setShowAccuracyConfirm(conv) : null} style={{ width: 36, height: 36, borderRadius: 8, background: conv.accuracy_report ? TEAL_LIGHT : BG, border: `1px solid ${BORDER}`, cursor: conv.extracted_text || conv.accuracy_report ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: conv.extracted_text || conv.accuracy_report ? 1 : 0.3 }}>
                ...
                <div style={{ position: 'relative' }}><div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch', scrollbarWidth: 'thin', scrollbarColor: `${TEAL} ${BORDER}` }}><table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 700 }}>
                ...
                </table></div><div style={{ height: 3, background: BORDER, borderRadius: 2, margin: '4px 0' }}><div style={{ height: '100%', width: '30%', background: TEAL, borderRadius: 2, opacity: 0.5 }} /></div></div>
              </div>
            </div>
          )}
