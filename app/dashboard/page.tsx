...
const SPLITTER_SPEEDS = [0.5, 1, 1.5, 1.75, 2, 2.25, 2.5, 2.75, 3, 3.25, 3.5, 3.75, 4]
function formatSplitterTime(t: number) {
  if (!isFinite(t)) return '0:00'
  const m = Math.floor(t / 60)
  const s = Math.floor(t % 60)
  return `${m}:${String(s).padStart(2, '0')}`
}
const splitterTransportBtnStyle: React.CSSProperties = { width: 36, height: 36, borderRadius: 10, border: '1px solid #ecebe8', background: '#f6f5f3', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15 }
...
export default function Dashboard() {
  ...
  React.useEffect(() => { splitterLoopStartRef.current = splitterLoopStart }, [splitterLoopStart])
  ...
  React.useEffect(() => {
    ...
    import('wavesurfer.js').then(({ default: WaveSurfer }) => {
      ...
      ws.on('timeupdate', (t: number) => {
        setSplitterCurrentTime(t)
        if (splitterLoopingRef.current && t >= splitterLoopStartRef.current + 4) {
          ws.setTime(splitterLoopStartRef.current)
        }
      })
  ...
  function splitterSeek(delta: number) {
    ...
    const t = Math.min(Math.max(ws.getCurrentTime() + delta, 0), splitterDuration)
    ws.setTime(t)
  }
  function splitterTogglePlay() {
    splitterWSRef.current?.playPause()
  }
  function splitterStop() {
    const ws = splitterWSRef.current
    if (!ws) return
    ws.pause()
    ws.setTime(0)
  }
  function splitterToggleLoop() {
    setSplitterLooping(prev => {
      const next = !prev
      if (next) {
        const t = splitterWSRef.current?.getCurrentTime() || 0
        setSplitterLoopStart(t)
        splitterLoopStartRef.current = t
      }
      splitterLoopingRef.current = next
      return next
    })
  }
  React.useEffect(() => {
    if (page !== 'audiosplitter' || splitterState !== 'loaded') return
    function handleKey(e: KeyboardEvent) {
      const tag = (e.target as HTMLElement)?.tagName
      if (tag === 'INPUT' || tag === 'TEXTAREA') return
      if (e.key === 'ArrowLeft') { e.preventDefault(); splitterSeek(-5) }
      else if (e.key === 'ArrowRight') { e.preventDefault(); splitterSeek(5) }
      else if (e.key === 'ArrowDown') { e.preventDefault(); splitterTogglePlay() }
      else if (e.key === 'l' || e.key === 'L') { e.preventDefault(); splitterToggleLoop() }
    }
    window.addEventListener('keydown', handleKey)
    return () => window.removeEventListener('keydown', handleKey)
  }, [page, splitterState, splitterDuration])
...
<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={TEAL} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14,2 14,8 20,8"/><line x1="12" y1="18" x2="12" y2="12"/><polyline points="9,15 12,18 15,15"/></svg></button>
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                  <div style={{ width: '80vw', height: '80vh', background: '#fff', borderRadius: 16, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                    <div style={{ padding: '16px 24px', borderBottom: `1px solid ${BORDER}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <p style={{ fontSize: 14, fontWeight: 600, margin: 0 }}>{splitterFile.name}</p>
                    ...
                    <div style={{ borderTop: `1px solid ${BORDER}`, background: '#fff', padding: '12px 24px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
                        ...
                        <div className="rm-scrollbar" style={{ display: 'flex', gap: 4, overflowX: 'auto', maxWidth: '40%', paddingBottom: 2 }}>
                          {SPLITTER_SPEEDS.map(s => (
                            <button key={s} onClick={() => { setSplitterSpeed(s); splitterWSRef.current?.setPlaybackRate(s) }} style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, padding: '6px 7px', borderRadius: 8, border: `1px solid ${BORDER}`, background: splitterSpeed === s ? '#1a1a1a' : SURFACE, color: splitterSpeed === s ? '#fff' : MUTED, cursor: 'pointer', flexShrink: 0 }}>{s}x</button>
                          ))}
                        </div>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'center', gap: 18, marginTop: 10, paddingTop: 10, borderTop: `1px solid ${BORDER}`, fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, color: TEXT }}>
          ...
          {page === 'reports' && (
            <div>
              <div className="it-card" style={{ border: `1px solid ${BORDER}`, borderRadius: 18, boxShadow: SHADOW, overflow: 'hidden' }}>
                <div style={{ padding: '14px 20px', borderBottom: `1px solid ${darkMode ? 'rgba(255,255,255,.12)' : BORDER}`, display: 'flex', gap: 12 }}>
                  <input placeholder="Search by address..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} style={{ flex: 1, padding: '8px 12px', borderRadius: 8, border: `1px solid ${darkMode ? 'rgba(255,255,255,.15)' : BORDER}`, fontFamily: 'inherit', fontSize: 13, outline: 'none', background: darkMode ? 'rgba(255,255,255,.06)' : '#fff', color: darkMode ? '#f3f0ea' : TEXT }} />
                  <button onClick={() => setShowDeleteAll(true)} style={{ padding: '8px 14px', borderRadius: 8, border: `1px solid ${darkMode ? 'rgba(220,38,38,.4)' : '#FECACA'}`, background: darkMode ? 'rgba(220,38,38,.12)' : '#FEF2F2', color: darkMode ? '#FCA5A5' : '#DC2626', fontFamily: 'inherit', fontSize: 12, fontWeight: 600, cursor: 'pointer', flexShrink: 0 }}>Delete all</button>
                </div>
                {isMobile && (
                  <div>
                ...
                <div style={{ position: 'relative' }}><div onScroll={(e) => { const el = e.currentTarget; const fade = el.nextElementSibling as HTMLElement; if (fade) fade.style.opacity = el.scrollLeft + el.clientWidth >= el.scrollWidth - 5 ? '0' : '1' }} style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}><table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 700 }}>
                  <thead><tr style={{ background: darkMode ? 'rgba(255,255,255,.04)' : BG }}>{(toolTab === 'audio' ? ['Property','Property Size','Furn/Unfurn','Audio Length','Conv. Time','Cost','By','Rating','Date',''] : ['Property','Rooms','Conv. Time','Cost','By','Rating','Date','']).map(h => <th key={h} style={{ fontSize: 11, fontWeight: 600, color: darkMode ? '#a49f92' : HINT, textTransform: 'uppercase', padding: '10px 20px', textAlign: 'left', borderBottom: `1px solid ${darkMode ? 'rgba(255,255,255,.12)' : BORDER}` }}>{h}</th>)}</tr></thead>
                  <tbody>{conversions.filter(c => !searchQuery || (c.address||'').toLowerCase().includes(searchQuery.toLowerCase())).filter(c => toolTab === 'audio' ? c.type === 'audio' : c.type !== 'audio').map(c => (<tr key={c.id} style={{ borderBottom: `1px solid ${BORDER}` }}>
                    <td style={{ padding: '12px 20px', fontSize: 11, fontWeight: 500, maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: darkMode ? '#f3f0ea' : TEXT }}>{fmtAddr(c.address)}</td>
