...
export default function Dashboard() {
  ...
  async function loadConversions(token: string) {
    ...
    const { data } = await client.from('conversions').select('*').order('created_at', { ascending: false }).limit(20)
    ...
    try {
      ...
      if (accessToken) {
        await saveConversion({
          address: data.address || selectedFile.name,
          rooms: rooms.length,
          items: totalItems,
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ background: SURFACE, borderBottom: `1px solid ${BORDER}`, padding: '0 32px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, background: TEAL_LIGHT, borderRadius: 20, padding: '6px 14px', fontSize: 13, fontWeight: 600, color: TEAL_DARK }}>{conversions.length} conversions</div>
            ...
            <div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,minmax(0,1fr))', gap: 16, marginBottom: 28 }}>
                {[
                  ...
                  ['Total spent', `£${totalSpent.toFixed(2)}`, '@ £3.50 per report'],
                  ['Avg. time', conversions.length > 0 ? `${avgTime}s` : '—', 'per conversion'],
                  ['Est. saving', `£${(conversions.length * 15).toFixed(2)}`, 'vs. external typist'],
                ].map(([label,val,sub]) => (
              ...
              <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 14, overflow: 'hidden' }}>
                ...
                {conversions.length === 0 ? (
                  <div style={{ padding: 40, textAlign: 'center', color: HINT }}>
                    <p style={{ fontSize: 14, marginBottom: 8 }}>No conversions yet</p>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead><tr style={{ background: BG }}>
                      {['Property','Rooms','Items','Time','Cost','Date'].map(h => <th key={h} style={{ fontSize: 11, fontWeight: 600, color: HINT, textTransform: 'uppercase', letterSpacing: 0.8, padding: '10px 20px', textAlign: 'left', borderBottom: `1px solid ${BORDER}` }}>{h}</th>)}
                    </tr></thead>
                    <tbody>
          ...
          {page === 'reports' && (
            <div>
              <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 14, overflow: 'hidden' }}>
                ...
                {conversions.length === 0 ? (
                  <div style={{ padding: 40, textAlign: 'center', color: HINT }}>
                    <p style={{ fontSize: 14 }}>No conversions yet — convert your first PDF to see it here.</p>
                  </div>
                ) : (
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          {page === 'settings' && (
            <div style={{ maxWidth: 600 }}>
              ...
              <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 14, padding: 24, marginBottom: 16 }}>
                <p style={{ fontSize: 13, fontWeight: 600, marginBottom: 16 }}>Account</p>
