          ...
          {page === 'reports' && (
            <div>
              <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 14, overflow: 'hidden' }}>
                    ...
                    {conversions.filter(c => !searchQuery || (c.address||'').toLowerCase().includes(searchQuery.toLowerCase())).map(conv => (
                      <div key={conv.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderBottom: `1px solid ${BORDER}` }}>
                        ...
                        <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                          ...
                          <button title={conv.accuracy_report ? 'View accuracy report' : (conv.extracted_text ? 'Generate accuracy report — £1.50' : 'No source data')} onClick={() => conv.accuracy_report ? setViewingReport(conv) : conv.extracted_text ? setShowAccuracyConfirm(conv) : null} style={{ width: 36, height: 36, borderRadius: 8, background: conv.accuracy_report ? TEAL_LIGHT : BG, border: `1px solid ${BORDER}`, cursor: conv.extracted_text || conv.accuracy_report ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: conv.extracted_text || conv.accuracy_report ? 1 : 0.3 }}>
                ...
                <div style={{ overflowX: isMobile ? 'auto' : 'visible' }}><table style={{ width: '100%', borderCollapse: 'collapse', minWidth: isMobile ? 700 : 'auto' }}>
